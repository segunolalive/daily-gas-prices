...
class GasPrice:
    ...
    def get_daily_prices_csv(self):
        ...
        logging.info('SUCCESS! Daily Gas Prices Generated at {0}'.format(
    ...
    def __get_page(self):
        ...
        try:
            ...
            if req.status_code == 200:
                ...
                message = 'ERROR. Server returned a status code of ' + \
                    str(req.status_code)
                logging.warning(message)
        except Exception as e:
            message = 'An Error occured while fetching the html page. ' + \
                str(e)
    ...
    def __get_start_date(self, string):
        start_date = string.text.replace('\xa0\xa0', '').replace(
            '- ', ' ').replace('-', ' ').split(' ')[0:3]
        return self.normalise_date(start_date)

    def normalise_date(self, string):
        return datetime.datetime.strptime('-'.join(string), '%Y-%b-%d')
